...
# Word Beam Search decoding implementation
def word_beam_search_decoder(predictions, classes, beam_width=10, alpha=1.0, beta=0.0):
    char_list = ''.join(list(classes.keys()))
    predictions = np.swapaxes(predictions, 1, 2)
    wbs = WordBeamSearch(beam_width, 'Words', alpha, beta, corpus, char_list)
    decoded = wbs.compute(predictions)
    return decoded
...
# Decode using Word Beam Search
corpus = "\n".join(lexicon)  # Assuming lexicon contains all words in the corpus
decoded = word_beam_search_decoder(predictions_probabilities, classes)
